...
model = CharLSTM(num_chars=67).to(device)
...
def train(currepoch, epoch):
    ...
    for batch_idx in range(len(dataloader)):
        ...
        with open("./logs/lstm_train_loss.log", "a+") as lfile:
            lfile.write("{}\n".format(train_loss / total))

        with open("./logs/lstm_train_acc.log", "a+") as afile:
        ...
        torch.save(model.state_dict(), './weights_lstm/network_train.ckpt')
        with open("./information/network_lstm_info.txt", "w+") as f:
    ...
    torch.save(model.state_dict(), './checkpoints_lstm/network_train_epoch_{}.ckpt'.format(currepoch + 1))
