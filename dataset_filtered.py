...
class HumorDataset(Dataset):
    def __init__(self, 
        ...
        if(load):
            ...
            file_handler = open('./pickle/train_set_filtered.pkl', 'wb+')
            pickle.dump((self.train_data, self.train_label), file_handler)
            file_handler.close()

            file_handler = open('./pickle/test_set_filtered.pkl', 'wb+')
            pickle.dump((self.test_data, self.test_label), file_handler)
            file_handler.close()

            file_handler = open('./pickle/mapping_char2num_filtered.pkl', 'wb+')
            pickle.dump(char2num, file_handler)
            file_handler.close()

            file_handler = open('./pickle/mapping_num2char_filtered.pkl', 'wb+')
            pickle.dump(num2char, file_handler)
            file_handler.close()

        else:
            if(not self.test):
                file_handler = open('./pickle/train_set_filtered.pkl', 'rb+')
                self.train_data, self.train_label = pickle.load(file_handler)
                file_handler.close()
            else:
                file_handler = open('./pickle/test_set_filtered.pkl', 'rb+')
