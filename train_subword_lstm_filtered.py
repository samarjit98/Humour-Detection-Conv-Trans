...
def train(currepoch, epoch):
    ...
    for batch_idx in range(len(dataloader)):
        ...
        with open("./logs/subwordlstm_train_loss_filtered.log", "a+") as lfile:
            lfile.write("{}\n".format(train_loss / total))

        with open("./logs/subwordlstm_train_acc_filtered.log", "a+") as afile:
        ...
        torch.save(model.state_dict(), './weights_subwordlstm/network_train_filtered.ckpt')
        with open("./information/network_subwordlstm_info_filtered.txt", "w+") as f:
    ...
    torch.save(model.state_dict(), './checkpoints_subwordlstm/network_train_filtered_epoch_{}.ckpt'.format(currepoch + 1))
...
def test(currepoch, epoch):
    ...
    dataloader = iter(dataloader)
    print('\n=> Humor Testing Epoch: {}'.format(currepoch))
    ...
    with torch.no_grad():
        for batch_idx in range(len(dataloader)):
            ...
            with open("./logs/subwordlstm_test_loss_filtered.log", "a+") as lfile:
                lfile.write("{}\n".format(test_loss / total))

            with open("./logs/subwordlstm_test_acc_filtered.log", "a+") as afile:
                afile.write("{}\n".format(correct / total))
