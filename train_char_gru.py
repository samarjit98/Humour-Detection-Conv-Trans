...
model = CharGRU(num_chars=67).to(device)
...
def train(currepoch, epoch):
    ...
    for batch_idx in range(len(dataloader)):
        ...
        with open("./logs/gru_train_loss.log", "a+") as lfile:
            lfile.write("{}\n".format(train_loss / total))

        with open("./logs/gru_train_acc.log", "a+") as afile:
        ...
        torch.save(model.state_dict(), './weights_gru/network_train.ckpt')
        with open("./information/network_gru_info.txt", "w+") as f:
    ...
    torch.save(model.state_dict(), './checkpoints_gru/network_train_epoch_{}.ckpt'.format(currepoch + 1))
...
def test(currepoch, epoch):
    ...
    with torch.no_grad():
        for batch_idx in range(len(dataloader)):
            ...
            with open("./logs/gru_test_loss.log", "a+") as lfile:
                lfile.write("{}\n".format(test_loss / total))

            with open("./logs/gru_test_acc.log", "a+") as afile:
                afile.write("{}\n".format(correct / total))
